...
Options: `--port <number>` (default 4040), `--open` (auto-open browser), `--ide <name|cmd>` (IDE for opening worktrees), `--help`.
...
- **`src/server.ts`** — Hono app with `createBunWebSocket`. Tracks WebSocket clients in a `Set`. The watcher's `onUpdate` callback broadcasts state to all clients. New connections receive the current state immediately in `onOpen`. Endpoints: `GET /api/state` (full dashboard state), `GET /api/merged` (slim list of merged worktrees for cleanup automation), `POST /api/open` (open worktree in configured IDE).

- **`src/config.ts`** — Reads `~/.config/work-for-the-trees/config.json` for persistent settings (currently `ide`). Returns `{}` if file doesn't exist.

- **`src/ide.ts`** — IDE preset map and command resolution. `resolveIdeCommand` turns a preset name or raw command template into a `Bun.spawn` argv array. `getIdeName` maps a specifier to its preset key or `"custom"`.

- **`src/index.ts`** — CLI entry point. Validates each repo path is a git repo, loads config file, resolves IDE setting (CLI flag overrides config), then wires `createApp` → `start`.
